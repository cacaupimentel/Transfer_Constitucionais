...
datesStart = pandas.date_range(start=menordata, end=maiordata, freq = 'YS')
...
datapop = pandas.merge(pandas.DataFrame(dataset_list, columns=["idMuni", "Municipio", "Ano", "Populacao", "Indice"]), 
                   pandas.DataFrame(dtset_lst, columns=["idMuni", "UF", "Ano", "PopUF", "Indice"]), 
                   how = 'outer')
datapop['Populacao'] = pandas.to_numeric(datapop['Populacao'], errors='coerce')

# Fazendo um Group by no ano para ter o valor da população das capitais
datapop = pandas.merge(pandas.DataFrame(datapop.groupby('Ano')['Populacao'].sum(), columns=["Populacao"]).reset_index().rename(columns={'Populacao': 'PopCapitais'}),
                   datapop,
                   how='outer')
